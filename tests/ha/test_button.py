"""Scene button tests: per-line scene recall buttons."""
...
from custom_components.lunatone_dali2_iot4.button import (  # noqa: E402
    LunatoneSceneButton,
)
...
async def test_no_scene_buttons_when_unconfigured(coordinator):
...
async def test_scene_button_identity(coordinator, config_entry):
    btn = LunatoneSceneButton(coordinator, config_entry, 0, 3)
    assert btn.unique_id == f"{config_entry.entry_id}_line0_scene3"
    assert btn.device_info["identifiers"] == {
        ("lunatone_dali2_iot4", f"{config_entry.entry_id}_line0_scene3")
    }
    assert btn.device_info["name"] == "DALI Line 0 Scene 3"


async def test_scene_button_press_broadcasts_scene(
...
async def test_scene_button_lists_members_and_levels(coordinator, config_entry):
    # device gw_id 1 is on line 0, address 20
    _configure_scene(coordinator, 1, 0, pct=33.0)
    btn = LunatoneSceneButton(coordinator, config_entry, 0, 0)
